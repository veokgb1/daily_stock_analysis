...
def _render_snapshot_card(snap: Dict, prefix: str) -> None:
    is_market = snap.get("code") == "__market__"
    display_name = "📊 大盘复盘" if is_market else f"{snap.get('name') or '—'}（{snap.get('code') or '—'}）"
    ...
    if not is_market:
        with h_push:
            if st.button("📥", key=f"push_{prefix}_{snap['id']}", help="送入首页分析池", use_container_width=False, type="secondary"):
    ...
    if st.session_state.history_delete_pending_id == snap["id"]:
        ...
        with _dc1:
            if st.button(
                "🚨 确认隐藏",
                key=f"confirm_del_{prefix}_{snap['id']}",
        ...
        with _dc2:
            if st.button(
                "取消",
                key=f"cancel_del_{prefix}_{snap['id']}",
                use_container_width=False,
                type="secondary",
            ):
                st.session_state.history_delete_pending_id = None
...
def _render_batch_summary_table(batch_snaps: List[Dict]) -> None:
...
def _build_batch_download_text(run_id: str, batch_snaps: List[Dict]) -> str:
...
def _build_batch_debug_log(run_id: str, batch_snaps: List[Dict]) -> str:
...
def _build_batch_schema_json(run_id: str, batch_snaps: List[Dict]) -> str:
    payload = {
        "run_id": run_id,
...
def _render_batch_reports(run_id: str, batch_snaps: List[Dict]) -> None:
    market_reports = [snap for snap in batch_snaps if snap.get("code") == "__market__"]
    stock_reports = [snap for snap in batch_snaps if snap.get("code") != "__market__"]
    ...
    if stock_reports:
        with st.expander("🧾 个股报告", expanded=False):
            for idx, snap in enumerate(stock_reports, start=1):
                st.markdown(_card_title_html(f"{snap.get('name') or '—'}（{snap.get('code') or '—'}）"), unsafe_allow_html=True)
                st.markdown(snap.get("report_md") or "_无报告内容_")
    ...
    st.download_button(
        "⬇️ 下载全量报告",
        data=_build_batch_download_text(run_id, batch_snaps),
    ...
    with st.expander("🔒 机要区 (调试日志与 JSON)", expanded=False):
        ...
        with debug_col:
            st.download_button(
                "下载 API 原始 Log",
                data=_build_batch_debug_log(run_id, batch_snaps),
        ...
        with json_col:
            st.download_button(
                "下载 Schema JSON",
                data=_build_batch_schema_json(run_id, batch_snaps),
...
def _render_batch_expanders(filtered_snaps: List[Dict], recent_limit: int) -> None:
    ...
    for run_id in ordered_run_ids:
        ...
        batch_snaps = sorted(batch_snaps, key=lambda snap: (snap.get("code") == "__market__", snap.get("code") or ""))
        ...
        with st.expander(label, expanded=False):
            st.markdown(_meta_html(f"run_id: {run_id}"), unsafe_allow_html=True)
            _render_batch_summary_table(batch_snaps)
            st.divider()
...
# ── 列表卡片模式（默认视图）──────────────────────────────────────────────────
    ...
    st.markdown(
        _meta_html("未精确锁定单只股票时，按分析批次折叠展示历史结果。"),
        unsafe_allow_html=True,
    )
    st.markdown("")
